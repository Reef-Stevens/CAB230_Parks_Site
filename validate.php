...
function validateEmail(&$errors, $field_list, $field_name) {
  ...
  if (!isset($field_list[$field_name])|| empty($field_list[$field_name])) {
    $errors[$field_name] = 'Required. Must be in format xx@xx.xx';
...
function validateName(&$errors, $field_list, $field_name) {
  ...
  if (!isset($field_list[$field_name])|| empty($field_list[$field_name])) {
    $errors[$field_name] = 'Required. Must be between 2 and 20 letters long.';
  } else if (!preg_match($pattern, $field_list[$field_name])) {
...
function validateAge(&$errors, $field_list, $field_name) {
  ...
  if (!isset($field_list[$field_name])|| empty($field_list[$field_name])) {
    $errors[$field_name] = 'Required. Must be between 18 and 99.';
} else if (!preg_match($pattern, $field_list[$field_name])) {
...
function validatePass(&$errors, $field_list, $field_name) {
  ...
  if (!isset($field_list[$field_name])|| empty($field_list[$field_name])) {
    $errors[$field_name] = 'Required. Must be at least 8 char, with at least 1 uppercase, 1 lowercase and 1 number.';
  } else if (!preg_match($pattern, $field_list[$field_name])) {
...
function validateDate(&$errors, $field_list, $field_name) {
  ...
  if (!isset($field_list[$field_name])|| empty($field_list[$field_name])) {
    $errors[$field_name] = 'Required. Must in in format DD/MM/YYYY';
  } else if (!preg_match($pattern, $field_list[$field_name])) {
